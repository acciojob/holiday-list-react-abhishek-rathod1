...
const App = () => {
  ...
  let indianPlace = cityList.filter((item) => item.country == "India");
  console.log(indianPlace);

  return (
    <div id="main">
      {/* Do not remove the main div */}
      <div className="container">
        <h1>List of Indian Tourist Places</h1>

      <ol>
        {indianPlace.map((item, index) => (
          <li key={'location'+(index+1)}>
            <Card name={item.name} country={item.country}></Card>
          </li>
        ))}
      </ol>
        </div>
